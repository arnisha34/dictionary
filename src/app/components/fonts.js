...
export const Fonts = () => {
  ...
  return(
    <div id="fontOptions" className="text-md font-bold pr-5 self-center relative min-w-[175px]">
      ...
      <ul className={`${isOpen===false?'hidden':''} transition-all transition-discrete duration-300 font-option flex flex-col gap-4 bg-white mt-3 pl-5 pr-10 py-5 rounded-2xl drop-shadow-lg cursor-pointer hover:sky-600 absolute dark:bg-sky-950 dark:text-white`} onMouseLeave={handleMouseLeave}>
      {listItems}
      </ul>
    </div>
  )
